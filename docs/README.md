# Mirante4D

Mirante4D is a web viewer for 4D (3D + time) lattice light-sheet microscopy datasets.

Try it [here](https://kirchhausenlab.github.io/llsm_viewer/)!

## Program docs

- [Sparse Segmentation Hard Cutover](./sparse-segmentation/README.md)
- [Annotate Overhaul](./annotate-overhaul/README.md)
- [Three r184 Hard Cutover](./three-r184-hard-cutover/README.md)
- [Orthographic Projection Mode Program](./orthographic-projection-mode/README.md)
- [Projection-Aware Residency Refactor](./projection-aware-residency/README.md)
- [GPU Segmentation Slice Rendering](./gpu-segmentation-slice-rendering/README.md)
- [VR Input Regression Checklist](./vr-input-regression-checklist.md)

## Install locally

Requirements:
- Node.js
- npm

```bash
npm install
npm run dev
```

The development server runs at `http://localhost:5173`.

## Production preview

```bash
npm run build
npm run preview
```
