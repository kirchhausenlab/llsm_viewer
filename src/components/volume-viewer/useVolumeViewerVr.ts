    ...
    for (let index = 0; index < 2; index++) {
      ...
      entry.onSelectStart = () => {
        ...
        if (entry.activeUiTarget?.type === 'volume-translate-handle') {
          ...
          if (handle && volumeRootGroup) {
            ...
            vrHandleQuaternionTempRef.current
              .copy(vrHandleWorldPointRef.current)
              .sub(vrHandleSecondaryPointRef.current);
            const directionLength = vrHandleQuaternionTempRef.current.length();
            if (directionLength > 1e-6) {
              vrHandleQuaternionTempRef.current.divideScalar(directionLength);
              const userScale = Math.max(volumeUserScaleRef.current, 1e-6);
              const baseLength = directionLength / userScale;
              entry.volumeScaleState = {
                direction: vrHandleQuaternionTempRef.current.clone(),
                baseLength,
              };
            } else {
              entry.volumeScaleState = null;
        ...
        if (
          ...
          if (volumeRootGroup) {
            vrHandleSecondaryPointRef.current.copy(volumeRootCenterUnscaledRef.current);
            volumeRootGroup.localToWorld(vrHandleSecondaryPointRef.current);
            vrHandleQuaternionTempRef.current.copy(entry.rayOrigin).sub(vrHandleSecondaryPointRef.current);
            if (entry.activeUiTarget.type === 'volume-yaw-handle') {
              vrHandleQuaternionTempRef.current.y = 0;
              let initialAngle = volumeYawRef.current;
              const yawBasisForward = new THREE.Vector3();
              const yawBasisRight = new THREE.Vector3();
              computeViewerYawBasis(rendererInstance, camera, yawBasisForward, yawBasisRight);
              if (vrHandleQuaternionTempRef.current.lengthSq() > 1e-6) {
              ...
              vrHandleQuaternionTempRef.current.x = 0;
              let initialAngle = volumePitchRef.current;
              const pitchBasisForward = new THREE.Vector3();
              const pitchBasisRight = new THREE.Vector3();
              computeViewerYawBasis(rendererInstance, camera, pitchBasisForward, pitchBasisRight);
              if (vrHandleQuaternionTempRef.current.lengthSq() > 1e-6) {
                const forwardComponent = vrHandleQuaternionTempRef.current.dot(pitchBasisForward);
                initialAngle = Math.atan2(vrHandleQuaternionTempRef.current.y, forwardComponent);
              }
              entry.volumeRotationState = {
                mode: 'pitch',
      ...
      entry.onSelectEnd = () => {
        ...
        if (activeTarget?.type === 'playback-play-toggle') {
          ...
          if (layerState) {
            if (region.sliderKey === 'opacity') {
              vrCallbacks?.onTrackOpacityChange?.(region.channelId, layerState.settings.opacity);
            } else if (region.sliderKey === 'lineWidth') {
              vrCallbacks?.onTrackLineWidthChange?.(
                region.channelId,
                layerState.settings.lineWidth,
              );
            } else if (region.sliderKey === 'windowMin') {
              vrCallbacks?.onLayerWindowMinChange?.(region.layerKey, layerState.settings.windowMin);
              vrCallbacks?.onLayerWindowMaxChange?.(region.layerKey, layerState.settings.windowMax);
            } else if (region.sliderKey === 'windowMax') {
              vrCallbacks?.onLayerWindowMinChange?.(region.layerKey, layerState.settings.windowMin);
              vrCallbacks?.onLayerWindowMaxChange?.(region.layerKey, layerState.settings.windowMax);
            } else if (region.sliderKey === 'contrast') {
              vrCallbacks?.onLayerContrastChange?.(region.layerKey, layerState.settings.contrast);
            } else if (region.sliderKey === 'brightness') {
              vrCallbacks?.onLayerBrightnessChange?.(
                region.layerKey,
                layerState.settings.brightness,
              );
            } else if (region.sliderKey === 'xOffset') {
              vrCallbacks?.onLayerOffsetChange?.(
                region.layerKey,
                'x',
                layerState.settings.xOffset,
              );
            } else if (region.sliderKey === 'yOffset') {
              vrCallbacks?.onLayerOffsetChange?.(
                region.layerKey,
                'y',
                layerState.settings.yOffset,
              );
            }
          }
          if (hud) {
            renderVrChannelsHudRef.current?.(hud, state);
          }
