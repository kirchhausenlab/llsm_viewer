...
export type UseVolumeViewerVrResult = {
  ...
  initializeVrHudScene: (scene: THREE.Scene) => void;
  ...
  startImmersiveRenderLoop: (
    renderer: THREE.WebGLRenderer,
    renderLoop: XRFrameRequestCallback,
  ) => void;
  cleanupImmersivePresentation: (renderer: THREE.WebGLRenderer | null) => void;
...
export function useVolumeViewerVr({
  ...
  const disposeHudGroup = (
    ...
    if ('labelTexture' in hud && hud.labelTexture) {
      hud.labelTexture.dispose();
    }
    if ('panelTexture' in hud && hud.panelTexture) {
      hud.panelTexture.dispose();
    }
    placementRef.current = null;
  };

  const disposeVrHudScene = useCallback(() => {
    disposeHudGroup(vrPlaybackHudRef.current, vrPlaybackHudPlacementRef);
    vrPlaybackHudRef.current = null;

    disposeHudGroup(vrChannelsHudRef.current, vrChannelsHudPlacementRef);
    vrChannelsHudRef.current = null;

    disposeHudGroup(vrTracksHudRef.current, vrTracksHudPlacementRef);
  ...
  const initializeVrHudScene = useCallback<UseVolumeViewerVrResult['initializeVrHudScene']>(
    (scene) => {
      const playbackHud = createVrPlaybackHud();
      if (playbackHud) {
  ...
  const registerDesktopPointerHandlers = useCallback<
    UseVolumeViewerVrResult['registerDesktopPointerHandlers']
  >(
    (renderer, onTrackSelectionToggle) => {
      const domElement = renderer.domElement;
      const pointerVector = new THREE.Vector2();
      const raycaster = new (THREE as unknown as { Raycaster: new () => RaycasterLike }).Raycaster();
      raycaster.params.Line = { threshold: 0.02 } as unknown as typeof raycaster.params.Line;
      (raycaster.params as unknown as { Line2?: { threshold: number } }).Line2 = { threshold: 0.02 };
      ...
      return () => {
        ...
        if (activePointerState) {
          try {
            domElement.releasePointerCapture(activePointerState.pointerId);
          } catch (error) {
            // Ignore errors from unsupported pointer capture
          }
        }
        if (controlsRef.current && activePointerState) {
  ...
  const startImmersiveRenderLoop = useCallback<UseVolumeViewerVrResult['startImmersiveRenderLoop']>(
    (renderer, renderLoop) => {
  ...
  const cleanupImmersivePresentation = useCallback<
    UseVolumeViewerVrResult['cleanupImmersivePresentation']
  >(
    (renderer) => {
      restoreVrFoveation();
